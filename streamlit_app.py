# ==============================================================================
# CÓDIGO DEL SIMULADOR v17 (VERSIÓN FINAL CON CORRECCIONES DE UI)
...
PALETA_COLORES = {
    ...
    'texto_claro': '#FFFFFF',
    'reserva_ingreso': '#00838F',
    'general_ingreso': '#B2EBF2',
    'reserva_ascenso': '#FF8F00',
    'general_ascenso': '#FFECB3',
}
CREDITOS_SIMULADOR = (
    "Código original de: Edwin Arturo Ruiz Moreno - Comisionado Nacional del Servicio Civil\n"
    "Derechos Reservados CNSC © 2025\n"
...
class GeneradorReporte:
    ...
    def generar_tabla_html(self) -> str:
        ...
        styles = [
            dict(selector="th", props=[("font-size", "12pt"), ("text-align", "center"), ("background-color", PALETA_COLORES['secundario']), ("color", PALETA_COLORES['texto_claro']), ("border", "1px solid " + PALETA_COLORES['secundario']), ("padding", "10px")]),
            dict(selector="td", props=[("font-size", "11pt"), ("text-align", "center"), ("border", "1px solid #ddd"), ("color", PALETA_COLORES['texto_principal']), ("padding", "8px")]),
            dict(selector="tr:hover", props=[("background-color", "#f5f5f5")]),
            dict(selector="tr:last-child", props=[("background-color", "#E0F2F1"), ("font-weight", "bold")])
        ]
    ...
    def crear_grafico_barras_apiladas(self) -> Optional[io.BytesIO]:
        ...
        for bar_group in (bars1, bars2):
            for bar in bar_group:
                width = bar.get_width()
                if width > 0:
                    ax.text(bar.get_x() + width / 2, bar.get_y() + bar.get_height() / 2, f'{int(width)}', ha='center', va='center', fontsize=11, weight='bold', color=PALETA_COLORES['texto_principal'])
    ...
    def crear_grafico_donut(self) -> Optional[io.BytesIO]:
        ...
        def func(pct, allvals):
            ...
            return f"{pct:.1f}%\n({absolute})"
        wedges, texts, autotexts = ax.pie(sizes, autopct=lambda pct: func(pct, sizes), startangle=90, colors=colors, pctdistance=0.8, wedgeprops=dict(width=0.4, edgecolor='w'))
...
# ==============================================================================
# INTERFAZ DE USUARIO CON STREAMLIT (DISEÑO RESTAURADO Y CORREGIDO)
# ==============================================================================
def main():
    ...
    with col1:
        try:
            # Se asume que 'logo.jpg' está en el mismo directorio que el script.
            st.image("logo.jpg", width=130)
        except Exception:
            st.warning("No se encontró 'logo.jpg'.")
    with col2:
        st.title("Simulador de Reserva de Plazas para PcD")
        st.markdown("Herramienta de apoyo para la aplicación de la **Ley 2418 de 2024**.")
    
    st.divider()

    # --- FORMULARIO DE ENTRADA ---
    with st.container():
        st.subheader("📝 1. Ingrese los Datos de la Entidad")
        c1, c2 = st.columns(2)
        with c1:
        ...
        st.subheader("🔀 2. Seleccione la Distribución de Vacantes")
        distribucion_tipo = st.radio(
            "Método de Distribución",
            options=['Automático (70/30)', 'Manual'],
            horizontal=True,
            help="La normativa sugiere un 30% para Ascenso y 70% para Ingreso. Puede ajustarlo manualmente."
        )
        ...
        with c3:
            if es_automatico:
                ...
                st.metric(label="Vacantes para Ascenso (30%)", value=vacantes_ascenso)
            else:
        ...
        with c4:
            ...
            st.metric(label="Vacantes para Ingreso (70%)", value=vacantes_ingreso, delta_color="off")
        
        if vacantes_ingreso < 0:
            st.error("El número de vacantes de ascenso no puede superar el total de la OPEC.")

        st.subheader("♿ 3. Verifique la Condición para Ascenso")
        ...
        if vacantes_ascenso > 0:
            respuesta_elegibilidad = st.radio(
                ...
                index=1,
                help="Si la respuesta es 'No', la reserva de ascenso será 0 por disposición legal."
            )
    ...
    # --- BOTÓN DE ACCIÓN Y RESULTADOS ---
    if st.button("🚀 Generar Simulación y Reporte", use_container_width=True, type="primary"):
        if not nombre_entidad.strip():
            st.error("⚠️ El nombre de la entidad es obligatorio.")
        elif vacantes_ingreso < 0:
            st.error("⚠️ Error en la distribución. Revise las vacantes.")
        else:
